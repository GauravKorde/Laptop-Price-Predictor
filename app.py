...
# 1. Load user data from file or create if not exists
def load_users():
...
def main_app():
    ...
    if st.button('📊 Predict Price'):
        ...
        if weight <= 0:
            ...
            # Prediction with a spinner
            with st.spinner('⏳ Predicting price...'):
                prediction = pipe.predict(query_df)
                predicted_price = int(np.exp(prediction[0]))  # Model was trained on log(price)
            ...
            # Generate and display a smart search prompt
            ...
            # Display the search prompt in a code block
            st.code(search_prompt, language='text')

            # Custom HTML for a reliable "Copy to Clipboard" button
            components.html(f"""
                <div>
    ...
    # --- Display Similar Laptops from Dataset ---
    st.markdown("---")
    st.markdown("#### 🔎 Similar Laptops from Dataset")

    # Filtering logic for finding similar laptops
    similar = (
